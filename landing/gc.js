/* Golden Coffee — shared front-end helpers (vanilla, no framework).
...
(function () {
  ...
  // --- backend / auth wiring -------------------------------------------------
  // The static landing site talks to the Golden Coffee hub (FastAPI on Railway).
  ...
  window.GC = {
    ...
    api: function (path, opts) {
      ...
      return fetch(resolveBackend() + path, cfg).then(function (res) {
        ...
        var err = new Error('Network error — could not reach Golden Coffee. Check your connection and try again.');
