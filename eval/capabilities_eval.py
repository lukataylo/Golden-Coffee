...
def eval_music_model() -> None:
    ...
    feats = features({"occupancy": 6, "queue_len": 2, "ts": ts_at(14)})
    ...
    probs = m.probabilities({"occupancy": 6, "queue_len": 1, "ts": ts_at(14)})
    ...
    toss = {"occupancy": 6, "queue_len": 1, "ts": ts_at(14)}
    ...
    rush = {"occupancy": 10, "queue_len": 5, "ts": ts_at(14)}
...
def eval_policy() -> None:
    ...
    acts = policy.decide({"occupancy": 6, "queue_len": 1,
                          "ts": ts_at(14)}, stb)
    sm = [a for a in acts if a.action == "set_music"]
...
def eval_schemas() -> None:
    ...
    scenarios = [
        {"occupancy": 9, "queue_len": 4, "ts": ts_at(13)},
        {"occupancy": 10, "queue_len": 1,
         "ts": ts_at(13), "tracks": [seated(i, 700 + i * 20) for i in range(3)]},
        {"occupancy": 2, "queue_len": 0, "ts": ts_at(20)},
    ]
    all_actions = []
    for sc in scenarios:
...
def eval_federated() -> None:
    ...
    try:
        ...
        fired = None
        for i in range(14):
            scene = {"occupancy": 5 + (i % 4), "queue_len": i % 3,
                     "ts": base + i * 1.0}
