...
Smart diffusers rarely have a clean public API, so this driver supports the two
setups that actually work at a hackathon, in priority order:

  1. HTTP webhook  — SCENT_WEBHOOK_URL: we POST {"intensity":N,"scent":"..."} to it.
     Works with Home Assistant webhooks, a Shelly/ESP relay, IFTTT, etc.
  2. Broadlink IR  — reuse the same RM4 blaster as the AC: learn the diffuser's
...
Env: SCENT_WEBHOOK_URL  OR  SCENT_IR_ON / SCENT_IR_OFF
